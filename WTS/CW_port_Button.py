...
def connect():
    global inst, inst1, inst2, inst3, rm
    ...
    try:
        inst2 = rm.open_resource('TCPIP0::192.168.100.254::inst4::INSTR',  
            write_termination = '\n', read_termination='\n')
    except:
    ...
    try:
        inst3 = rm.open_resource('TCPIP0::192.168.100.254::inst5::INSTR',  
            write_termination = '\n', read_termination='\n')
    except:
...
frequency = IntVar()
power = IntVar()
set_freq = Entry(connection, width=10, textvariable = frequency)
set_freq.grid(column=0,row=1)


set_power = Entry(connection, width=10, textvariable = power)
set_power.grid(column=1,row=1)
...
def ch1_capture_tx_1():
    ...
    inst1.write(":CONFigure:RFSA:GPRF:FREQuency 3305000000")
    ...
    log_box.see(tk.END)
    sleep(0.02)
    inst.write("SOURce:RFSG:GPRF:STATe OFF")
...
def ch1_gen_1():
    if set_freq.get() != '0' :
        ...
        #log_box.insert(tk.END, datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f') + "[VISA RECV]"+ opc + " \n")
        #log_box.see(tk.END)
        status = inst.query(":SOURce:RFSG:GPRF:STATUS?")
...
def ch2_gen_1():
    if set_freq.get() != '0' :
        #==RX Min PER[11B_11M_2442MHz(CH:7)_Ant1]==
        log_box.insert(tk.END, datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f') +"--------------SOURCE-----------\n")
        inst.write(":CONFigure:ATTenuation:LOSS:EXTernal 0, 2410, 2.00")
        #log_box.insert(tk.END, datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f') + "[VISA SEND]" + " :CONFigure:ATTenuation:LOSS:EXTernal 0, 2405, 2.00\n")
        inst.write("SOURce:RFSG:GPRF:LIST:WAVeform CW")
        #log_box.insert(tk.END, datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f') + "[VISA SEND]" + " SOURce:RFSG:GPRF:LIST:WAVeform CW\n")
        inst.write("SOURce:RFSG:GPRF:RFSettings:FREQuency 2410E6")
        #log_box.insert(tk.END, datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f') + "[VISA SEND]" + " SOURce:RFSG:GPRF:RFSettings:FREQuency 2405E6\n")
        inst.write("SOURce:RFSG:GPRF:ARB:IDLEtime -1")
        #log_box.insert(tk.END, datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f') + "[VISA SEND]" + " SOURce:RFSG:GPRF:ARB:IDLEtime -1\n")
        inst.write("SOURce:RFSG:GPRF:ARB:REPetition 0")
        #log_box.insert(tk.END, datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f') + "[VISA SEND]" + " SOURce:RFSG:GPRF:ARB:REPetition 0\n")
        inst.write("SOURce:RFSG:GPRF:RFSettings:LEVel -30.00")
        #log_box.insert(tk.END, datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f') + "[VISA SEND]" + " SOURce:RFSG:GPRF:RFSettings:LEVel -30.00\n")
        inst.write("SOURce:RFSG:GPRF:ARB:CYCLes -1")
        #log_box.insert(tk.END, datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f') + "[VISA SEND]" + " SOURce:RFSG:GPRF:ARB:CYCLes -1\n")
        inst.write("SOURce:RFSG:GPRF:STATe ON")
...
measurement.grid(column=0,row=2)
